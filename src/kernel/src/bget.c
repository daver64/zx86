...
void bectl(compact, acquire, release, pool_incr) int(*compact) _((bufsize sizereq, int sequence));
...
void bpoold(buf, dumpalloc, dumpfree) void *buf;
...
static void stats(when) char *when;
{
...
static void bshrink(buf) void *buf;
{
	if (((char *)buf) == bp)
	{
#ifdef EXPTRACE
