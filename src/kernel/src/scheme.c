...
static num num_add(num a, num b)
{
     ...
     {
          //	 printf("<scheme add doubles>[%lf][%lf]\n",num_rvalue(a),num_rvalue(b));
...
static int alloc_cellseg(scheme *sc, int n)
{
     ...
     for (k = 0; k < n; k++)
     {
          ...
          if (((uint32_t)cp) % adj != 0)
          {
               cp = (char *)(adj * ((uint32_t)cp / adj + 1));
          }
...
static void putchars(scheme *sc, const char *s, int len)
{
     ...
     if (pt->kind & port_file)
     {
          fwrite((void *)s, 1, len, pt->rep.stdio.file);
     }
     else
     {
          for (; len; len--)
...
static void atom2str(scheme *sc, pointer l, int f, char **pp, int *plen)
{
     ...
     else if (is_number(l))
     {
          ...
          if (f <= 1 || f == 10) /* f is the base for numbers if > 1 */
          {
               ...
               {
                    snprintf(p, STRBUFFSIZE, "%lf", rvalue_unchecked(l));
...
{
     /*	if(fin==stdin || fin==stdout || fin==stderr)
               return;
          int forig=ftell(fin);
